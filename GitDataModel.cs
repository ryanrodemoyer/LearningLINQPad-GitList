...
namespace LearningLINQPad.GitList
{
	...
	public object cmd_Commit
		{
			get
			{
				...
				var button = new LINQPad.Controls.Button("Commit This File...", _ =>
				{
					try
					{
	...
	public object cmd_Discard
	{
		get
		{
			...
			var button = new LINQPad.Controls.Button("Discard Changes", _ =>
			{
				try
				{
	...
	/// <summary>
	/// Button to stash all changes in the repository
	/// </summary>
	public object cmd_Stash
	{
		get
		{
			if (_repository == null) return null;
			...
			var button = new LINQPad.Controls.Button("Stash All", _ =>
			{
				try
				{
					...
					if (!status.IsDirty)
					{
						"No changes to stash".Dump();
						return;
					}
					
					// Prompt for stash message
					var message = Util.ReadLine($"Stash message (optional):", "WIP: changes");
					if (string.IsNullOrWhiteSpace(message))
					{
						"Stash cancelled - no message provided".Dump();
						return;
					}
					...
					// Create the stash (stashes all changes including untracked files)
					var stash = _repository.Stashes.Add(signature, message, StashModifiers.IncludeUntracked);
					
					$"✓ Stashed all changes".Dump();
					new { 
						Message = message,
						FilesStashed = status.Count()
					}.Dump("Stash Created");
				}
				catch (Exception ex)
				{
					$"Error stashing changes: {ex.Message}".Dump();
				}
			});
			return button;
		}
